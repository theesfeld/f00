...
8. **Projection Specimen Engine (org-wide goal).** f00 is an organic experience projected onto the display — not pages that “load.” **Every object is a projection:** field, boxes, header/footer rules, body type, chrome, splash plate. Scope is **everything** — hub + all project sites + all scenes.
   ...
   - **Every object is its own projection:** logo, cards, rules, type, field — each has its own seed and entropic rules (not one global wind with phase offsets). Species form stays f00; each specimen is independent. Continuous air per object can move its throw slightly without rebuilding identity. No primary object.
