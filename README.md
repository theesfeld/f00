...
<!-- bench-table:start -->
_CI / suite bench · `2026-07-24T13:08:21Z` · N=15 median · x86_64 · Linux 6.17.0-1020-azure_

| Tool | Command | GNU | f00 `--core` | vs GNU |
|------|---------|-----|--------------|--------|
| `true` | `f00-true --core` | 0.56 ms | **0.25 ms** | **~2.3×** |
| `basename` | `f00-basename --core /usr/bin/ls` | 0.81 ms | **0.25 ms** | **~3.2×** |
| `nproc` | `f00-nproc --core` | 0.87 ms | **0.30 ms** | **~2.9×** |
| `whoami` | `f00-whoami --core` | 0.95 ms | **0.31 ms** | **~3.0×** |
| `cat` | `f00-cat --core fixture.txt` | 0.83 ms | **0.29 ms** | **~2.8×** |
| `wc` | `f00-wc --core -l fixture.txt` | 0.88 ms | **0.34 ms** | **~2.6×** |
| `md5sum` | `f00-md5sum --core fixture.txt` | 1.27 ms | **0.39 ms** | **~3.3×** |
| `sha256sum` | `f00-sha256sum --core fixture.txt` | 1.24 ms | **0.45 ms** | **~2.8×** |
| `sort` | `f00-sort --core fixture.txt` | 1.36 ms | **0.88 ms** | **~1.5×** |
| `ls` | `f00-ls --core -1 dir` | 1.06 ms | **0.45 ms** | **~2.3×** |
<!-- bench-table:end -->
...
```bash
# pin version
curl -fsSL https://f00.sh/install.sh | F00_VERSION=v0.15.7 bash
...
Release assets for `v0.15.7` include tarball, **deb**, **rpm**, and **Arch** packages.
...
curl -fsSLO https://github.com/theesfeld/f00/releases/download/v0.15.7/f00_0.15.7_amd64.deb
sudo dpkg -i f00_0.15.7_amd64.deb

# Fedora / RHEL example
curl -fsSLO https://github.com/theesfeld/f00/releases/download/v0.15.7/f00-0.15.7-1.x86_64.rpm
sudo rpm -Uvh f00-0.15.7-1.x86_64.rpm

# Arch example (release package)
curl -fsSLO https://github.com/theesfeld/f00/releases/download/v0.15.7/f00-0.15.7-1-x86_64.pkg.tar.zst
sudo pacman -U f00-0.15.7-1-x86_64.pkg.tar.zst
```
