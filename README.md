...
| **Default** | Modern (color, table columns, chromed JSON/CSV) |
| **Icons** | Off by default · `--icons=ascii|glyph|emoji|nerd` opt-in · off under `--core` |
| **Scripts** | `--core` — strict coreutils-compatible presentation |
| **Engine** | Pure ASM multicall · ~650K static · no libc |
| **License** | MIT |
| **Status** | Released `v0.15.5` |
...
<!-- bench-table:start -->
_CI / suite bench · `2026-07-24T13:00:16Z` · N=15 median · x86_64 · Linux 6.17.0-1020-azure_

| Tool | Command | GNU | f00 `--core` | vs GNU |
|------|---------|-----|--------------|--------|
| `true` | `f00-true --core` | 0.62 ms | **0.30 ms** | **~2.1×** |
| `basename` | `f00-basename --core /usr/bin/ls` | 0.88 ms | **0.30 ms** | **~3.0×** |
| `nproc` | `f00-nproc --core` | 0.83 ms | **0.25 ms** | **~3.3×** |
| `whoami` | `f00-whoami --core` | 0.90 ms | **0.26 ms** | **~3.5×** |
| `cat` | `f00-cat --core fixture.txt` | 0.86 ms | **0.29 ms** | **~3.0×** |
| `wc` | `f00-wc --core -l fixture.txt` | 0.87 ms | **0.38 ms** | **~2.3×** |
| `md5sum` | `f00-md5sum --core fixture.txt` | 1.27 ms | **0.38 ms** | **~3.4×** |
| `sha256sum` | `f00-sha256sum --core fixture.txt` | 1.24 ms | **0.45 ms** | **~2.8×** |
| `sort` | `f00-sort --core fixture.txt` | 1.36 ms | **0.76 ms** | **~1.8×** |
| `ls` | `f00-ls --core -1 dir` | 1.08 ms | **0.45 ms** | **~2.4×** |
<!-- bench-table:end -->
...
```bash
# pin version
curl -fsSL https://f00.sh/install.sh | F00_VERSION=v0.15.5 bash
...
Release assets for `v0.15.5` include tarball, **deb**, **rpm**, and **Arch** packages.
...
curl -fsSLO https://github.com/theesfeld/f00/releases/download/v0.15.5/f00_0.15.5_amd64.deb
sudo dpkg -i f00_0.15.5_amd64.deb

# Fedora / RHEL example
curl -fsSLO https://github.com/theesfeld/f00/releases/download/v0.15.5/f00-0.15.5-1.x86_64.rpm
sudo rpm -Uvh f00-0.15.5-1.x86_64.rpm

# Arch example (release package)
curl -fsSLO https://github.com/theesfeld/f00/releases/download/v0.15.5/f00-0.15.5-1-x86_64.pkg.tar.zst
sudo pacman -U f00-0.15.5-1-x86_64.pkg.tar.zst
```
